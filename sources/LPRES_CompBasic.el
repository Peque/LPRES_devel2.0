...
-- NOTES: -- updated LSdLP 26/04/2020
-- AUTHOR: Pablo Sierra Heras & Juan Velasco Mariscal
-- CREATION DATE: 05/12/2014
-- LAST MODIFIED: 26/04/2020
--------------------------------------------------------------------------------

-- Libraries
USE MATH VERSION "3.2"   -- updated LSdLP 26/04/2020
...
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
-- Abstract component with 1 fluid inlet and 1 fluid outlet without combustion
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- Abstract component for all kinds of turbomachinery
--------------------------------------------------------------------------------
ABSTRACT COMPONENT Turbo IS_A FluidInFluidOut
   ...
   DECLS
      ...
      REAL rpm             UNITS u_rpm       "Rotational Speed"
...
--------------------------------------------------------------------------------
-- Abstract component for turbomachinery of incompressible fluids
--------------------------------------------------------------------------------
ABSTRACT COMPONENT LiqTurbo IS_A Turbo

   "Abstract component for turbomachinery of incompressible fluids"

   DECLS
      REAL dp              UNITS u_Pa        "Pressure difference"
      REAL tau_id          UNITS u_J_kg      "Ideal specific work"

   CONTINUOUS
      dp = f_out.pt - f_in.pt
      tau_id = -dp / rho(f_in.fluid)                  -- sign criterion: negative for pumps, positive for turbines

      tau_id - tau = cp(f_in.fluid) * (f_out.Tt - f_in.Tt)

END COMPONENT

--------------------------------------------------------------------------------
-- Abstract component for turbomachinery of compressible fluids
--------------------------------------------------------------------------------
ABSTRACT COMPONENT GasTurbo IS_A Turbo

   "Abstract component for turbomachinery of compressible fluids"

   DECLS
      REAL MFP             UNITS "m·s·K^.5"  "Mass flow parameter"

   CONTINUOUS
      tau = cp(f_in.fluid) * (f_in.Tt - f_out.Tt)     -- sign criterion: negative for compressors, positive for turbines
      MFP = f_in.W * sqrt(f_in.Tt) / f_in.pt
...
COMPONENT Turbine_ch IS_A GasTurbo
   ...
   DATA
      REAL eta_d = 0.9     UNITS no_units    "Design efficiency"
      REAL A_in = 0.01     UNITS u_m2        "Input area"

   DECLS
      REAL eta             UNITS no_units    "Efficiency"
      REAL alpha           UNITS no_units    "Total-to-total temperature ratio"
      REAL beta            UNITS no_units    "Total-to-total pressure ratio"
      REAL pi              UNITS no_units    "Expansion ratio"
      REAL theta           UNITS no_units    "Expansion temperature ratio"

   DISCRETE
      ASSERT (Gas == State(f_in.fluid)) FATAL "ONLY GASES CAN GO THROUGH THE CHOKED TURBINE!"

   CONTINUOUS
      alpha = f_out.Tt / f_in.Tt
      theta = 1 / alpha
      beta = f_out.pt / f_in.pt
      pi = 1 / beta

      eta = (1. - alpha) / (1. - beta**((gamma(f_in.fluid)-1.)/gamma(f_in.fluid)))
      eta = eta_d

      MFP = A_in * FGAMMA(f_in.fluid) / sqrt(R(f_in.fluid))
...
COMPONENT Turbine IS_A GasTurbo
   ...
   DATA
      REAL eta_d = 0.9     UNITS no_units    "Design efficiency"
      REAL phi_d = 0.5     UNITS no_units    "Design flow coefficient"
      REAL psi_d = 1.8     UNITS no_units    "Design loading coefficient"
      REAL r_m = 0.05      UNITS u_m         "Mean radius"
      REAL A_in = 0.01     UNITS u_m2        "Input area at the stator throat"
      REAL cos_2 = 0.7     UNITS no_units    "Cosine of the metallic angle at the stator outlet"
   ...
   DECLS
      ...
      REAL pi              UNITS no_units    "Expansion ratio"
      REAL theta           UNITS no_units    "Expansion temperature ratio"
      ALG REAL M_2 = 0.1   UNITS no_units    "Mach number at the stator outlet"
      REAL V_z2            UNITS u_m_s       "Axial component of the velocity at the stator outlet"
   ...
   CONTINUOUS
      ...
      MFP = A_in / sqrt(R(f_in.fluid)) * sqrt(gamma(f_in.fluid)) * min(M_2,1.) * (1. + (gamma(f_in.fluid) - 1.) / 2. * min(M_2,1.)**2)**(-(gamma(f_in.fluid) + 1.) / 2. / (gamma(f_in.fluid) - 1.))

      U = m.N * r_m
      phi = V_z2 / U
      psi = tau / U**2                              -- sign criterion: always positive

      V_z2 = cos_2 * M_2 * sqrt(gamma(f_in.fluid) * R(f_in.fluid) * f_in.Tt / (1. + (gamma(f_in.fluid) - 1.) / 2. * M_2**2))

      (psi + 1.) / phi = (psi_d + 1.) / phi_d       -- Euler Eq. for Turbomachinery
...
COMPONENT Compressor IS_A GasTurbo
   ...
   DATA
      ...
      REAL phi_d = 0.4     UNITS no_units    "Design flow coefficient"
   ...
   DECLS
      ...
      REAL phi             UNITS no_units    "Flow coefficient"
      REAL psi             UNITS no_units    "Loading coefficient"
      REAL pi              UNITS no_units    "Pressure ratio"
      REAL theta           UNITS no_units    "Temperature ratio"
      ALG REAL M_in = 0.1  UNITS no_units    "Inlet Mach number"
      REAL V_z1            UNITS u_m_s       "Axial component of the velocity at the stator inlet"
   ...
   CONTINUOUS
      theta = f_out.Tt / f_in.Tt
      pi = f_out.pt / f_in.pt

      eta = (pi**((gamma(f_in.fluid)-1.)/gamma(f_in.fluid)) - 1.) / (theta - 1.)
      eta = eta_d

      MFP = A_in / sqrt(R(f_in.fluid)) * sqrt(gamma(f_in.fluid)) * M_in * (1. + (gamma(f_in.fluid) - 1.) / 2. * M_in**2)**(-(gamma(f_in.fluid) + 1.) / 2. / (gamma(f_in.fluid) - 1.))

      U = m.N * r_m
      phi = V_z1 / U
      psi = -tau / U**2                             -- sign criterion: always positive

      V_z1 = M_in * sqrt(gamma(f_in.fluid) * R(f_in.fluid) * f_in.Tt / (1. + (gamma(f_in.fluid) - 1.) / 2. * M_in**2))
...
--------------------------------------------------------------------------------
COMPONENT Turbine_liq IS_A LiqTurbo

   "Liquid turbine"

   DATA
      REAL eta_d = 0.6     UNITS no_units    "Design efficiency"  -- ¿??? review typical values for rocket engines???
      REAL phi_d = 0.5     UNITS no_units    "Design flow coefficient"
      REAL psi_d = 1.8     UNITS no_units    "Design loading coefficient"
      REAL r_m = 0.05      UNITS u_m         "Mean radius"
      REAL A_in = 0.01     UNITS u_m2        "Input area"
   ...
   DECLS
      ALG REAL U = 500.    UNITS u_m_s       "Blade speed"  -- ¿??? maybe choose 'phi' as the variable for 'ALG' iterations???
      REAL eta             UNITS no_units    "Efficiency"
      REAL phi             UNITS no_units    "Flow coefficient"
      REAL psi             UNITS no_units    "Loading coefficient"

   DISCRETE
      ASSERT (Liquid == State(f_in.fluid)) WARNING "ONLY LIQUIDS CAN GO THROUGH THE LIQUID TURBINE!"  -- ¿??? just issue a mere 'WARNING'???

   CONTINUOUS
      tau = tau_id * eta                            -- sign criterion: positive for turbines
      eta = eta_d

      U = m.N * r_m
      phi = f_in.W / rho(f_in.fluid) / U / A_in
      psi = tau / U**2                              -- sign criterion: always positive
...
COMPONENT Pump IS_A LiqTurbo
   ...
   DISCRETE
      ASSERT (Liquid == State(f_in.fluid)) WARNING "ONLY LIQUIDS CAN GO THROUGH THE PUMP!"  -- ¿??? just issue a mere 'WARNING'???

   CONTINUOUS
      tau = tau_id / eta                            -- sign criterion: negative for pumps
      eta = eta_d
...
--------------------------------------------------------------------------------
-- Component that specifies the conditions of a fluid inlet/outlet in/out the system
--------------------------------------------------------------------------------
COMPONENT Inlet
   ...
   DATA
      ENUM LiquidsGases fluid = Air          "Working fluid name"
...
COMPONENT Outlet
   ...
   "Conditions of a fluid outlet out the system"

   PORTS
      IN Fluid f              "Inlet fluid port"
